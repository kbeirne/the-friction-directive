...
- The environment should have at least one location with special significance for multiple roles
...
**The player's role — what they know:**

- Describes what the player knows going into the game. This should be a part of the opening presentation.
...
- Skip scenes where nothing interesting happens near the player but simulate the remainder of the round for all other characters (can simplify with multiple NPCs asleep etc. but some characters should still be acting if it makes sense to the story)
...
- [List information relevant to the scene that exists in the Bible/Diary but that the player's character has no way of knowing yet]
```

**NOTE**: Apply the perception filter to suggested options, not just narration. Every option the player is offered must be motivated by something the player character has observed or could plausibly infer.
...
**The scratch space prevents the a very common failure mode: the narrator accidentally revealing information the player's character hasn't earned.** If something isn't on the "can observe" list, it must not appear in the player-facing narrative.
...
```
1. READ this framework document to remember how to enact the following steps correctly.
2. READ the Game Bible (check relevant character details)
3. WRITE the Diary entry (advance ALL NPCs, not just those near the player. Perform dead-end check on each NPC. Perform momentum check.)
...
- A round can involve multiple choices from the player.
- When a round has ended the narrator should state this and tell the player to type /round to continue
- When the narrator sees /round from the player, is is a reminder to read this `assymeytic_rpg_framework` document and follow EVERY step of the Round Structure. Once the narrator has completed up to step 6 (ensuring all information generated until this point is hidden) they should produce a checklist that confirms they have followed each of these steps. This should then be followed by the player-facing scenes and options. This allows the player to ensure that the game is running correctly.
- Every step that requires preamble should be obscured from the player except for the checklist.
...
- Describe physical tells: a hand drifting to a hidden weapon / an unusual facial expression / someone fidgeting near a door / eyes flicking to another room or door etc.
...
- Never tell the player how another character feels about them. Let them infer it through the characters actions.
...
- Connect to different plotlines or characters
...
- The epilogue should wrap up the player's story in a satisfying way.
- The epilogue should show consequences: who won, who lost, who escaped, who was changed
- The epilogue should then "zoom out" to reveal what the player missed. It should include every single NPC and aim to make a satisfying summary of the entire game's events by focusing more attention on the NPC's the player didn't see or interact with as much.
